...
source("gcp_query_tools/r/connect_analytics_helper_functions.R") 
...
table              <- tables$prom # table to be flattened (can only select one)
...
tmp_folder    <- paste0(table_folder,"tmp/")
...
```{r}
# Name schema files
schema_json  <- paste0(tmp_folder,table,"-schema.json")
schema_csv   <- paste0(tmp_folder,table,"-schema.csv")
...
source("gcp_query_tools/r/json_schema_to_csv.R")
...
out_json <- paste0(tmp_folder, table, "-lists-from-schema.json")
out_csv  <- paste0(tmp_folder, table, "-variables-from-schema.csv")
...
old_lists_json <- paste0(table_folder, table, "-lists.json"); old_lists_json

# name combined json file
union_lists_json <- paste0(tmp_folder, table, "-lists-union.json"); union_lists_json
...
old_vars_csv        <- paste0(table_folder, table, "-variables.csv")
union_vars_csv      <- paste0(tmp_folder, table, "-vars-union.csv"); union_vars_csv
...
new_lists_json    <- paste0(tmp_folder, table, "-lists-union-mod.json")
...
new_vars_csv      <- paste0(tmp_folder, table, "-variables-union-mod.csv")
...
source('gcp_query_tools/r/generate_flattening_query.R')
...
query_file_path <- 
  paste0(table_folder, query_config$output_table, "-", tier, ".sql") %>% 
  print()
...
```{r}
source('gcp_query_tools/r/update_scheduled_query.R')
