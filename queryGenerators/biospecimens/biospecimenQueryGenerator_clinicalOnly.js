...
const dataSource = {
  dev: 'nih-nci-dceg-connect-dev.Connect.biospecimen',
  stg: 'nih-nci-dceg-connect-stg-5519.Connect.biospecimen',
  prod: 'nih-nci-dceg-connect-prod-6d04.Connect.biospecimen',
};
const pathToConceptIdList = require('./biospecimen-lists');
let variables = fs
  .readFileSync('./biospecimen-variables-clinical.csv', 'utf8')
  .split(/\r?\n/)
  .map((v) => v.trim())
  .filter((v) => v.length > 0);
...
ORDER BY Connect_ID
  ...
  fs.writeFileSync(`./flatBiospecimen_JP-clinical-${tier}.txt`, content);
}
