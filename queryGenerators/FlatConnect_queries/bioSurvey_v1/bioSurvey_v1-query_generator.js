...
const pathToConceptIdList = require('./bioSurvey_v1-lists.json');
let variables = fs
  .readFileSync('./bioSurvey_v1-variables.csv', 'utf8')
  .split(/\r?\n/)
  .map((v) => v.trim())
  .filter((v) => v.length > 0);
...
ORDER BY Connect_ID
  ...
  fs.writeFileSync(`./FlatConnect.bioSurvey_v1_JP-${tier}.sql`, content);
}
