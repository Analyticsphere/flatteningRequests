...
const dataSource = {
  dev: 'nih-nci-dceg-connect-dev.Connect.module1_v2',
  stg: 'nih-nci-dceg-connect-stg-5519.Connect.module1_v2',
  prod: 'nih-nci-dceg-connect-prod-6d04.Connect.module1_v2',
};
const pathToConceptIdList = require('./module1_v2-lists.json');
let variables = fs
  .readFileSync('./module1_v2-variables.csv', 'utf8')
  .split(/\r?\n/)
  .map((v) => v.trim())
  .filter((v) => v.length > 0);
...
""";
  ...
  CREATE OR REPLACE TABLE FlatConnect.module1_v2_JP AS (
  WITH
  json_data AS (
    SELECT
      Connect_ID,
...
ORDER BY Connect_ID
  ...
  fs.writeFileSync(`./Flat.module1_v2_JP-${tier}.sql`, content);
}
