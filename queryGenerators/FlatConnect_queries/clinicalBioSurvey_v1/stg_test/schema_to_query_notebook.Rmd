...
table    <- "Connect.clinicalBioSurvey_v1"
schema   <- "clinicalBioSurvey_v1-schema.xlsx"
sheet    <- NULL
out_json <- "clinicalBioSurvey_v1-lists-from-schema.js" # destination file
out_csv  <- "clinicalBioSurvey_v1-variables-from-schema.csv"
...
#### 2. Combine json and csv files from manual queries with schema-informed query
...
```{r}
A <- "clinicalBioSurvey_v1-lists.json"
B <- "clinicalBioSurvey_v1.json"
C <- "clinicalBioSurvey_v1-lists-union.json"
...
A_csv <- "clinicalBioSurvey_v1-variables.csv"
B_csv <- "clinicalBioSurvey_v1-variables-from-schema.csv"
C_csv <- "clinicalBioSurvey_v1-variables-union.csv"
...
```{r}
output_filename  <- "clinicalBioSurvey_v1-lists-union-mod.json"
...
```{r}
out_file  <- "clinicalBioSurvey_v1-variables-union-new.csv"
...
echo $NEW_CSV  
echo $NEW_JSON
PAR_DIR_CSV_NAME="clinicalBioSurvey_v1-variables.csv"
PAR_DIR_JSON_NAME="clinicalBioSurvey_v1-lists.json"
...
node ../clinicalBioSurvey_v1-query_generator.js
```

**The code below is in development.**

\#### 7. Update scheduled query using bq CLI

```{R}
# Define parameters
project          <- "nih-nci-dceg-connect-dev"
service_account  <- "qa-qc-dev@nih-nci-dceg-connect-dev.iam.gserviceaccount.com"
query_file       <- "../FlatConnect.participants_JP-dev.sql"
sched_query_name <- "FlatConnect.participants_JP"

query_str <- readr::read_file(query_file) # read as single string

query_str

# Set environment variables
Sys.setenv(PROJECT_ID = project)
Sys.setenv(GCP_SERVICE_ACCOUNT = service_account)
Sys.setenv(GCP_QUERY = query_str)
Sys.setenv(DISPLAY_NAME = sched_query_name)
```

```{R}
str1        <- '{"query":'
str2        <- '"write_disposition":"WRITE_TRUNCATE"}'
params_str  <- paste0(str1, query_str, str2)
cat(params_str)

```

```{bash}
# Resource name: projects/1061430463455/locations/us/transferConfigs/6377e882-0000-2736-b618-240588723f18
str0="'"
str1='{"query":'
str2='}'
str3="'"
PARAMS_STR=$str0$str1$GCP_QUERY$str2$str3
# echo $PARAMS_STR

bq ls -j --project_id $PROJECT_ID > /dev/null

bq update \
--transfer_config \
--display_name='FlatConnect.participants_JP' \
--params=$PARAMS_STR \
...
#--params=$PARAMS_STR
#--display_name='FlatConnect.participants_JP'
#--service_account_name=$GCP_SERVICE_ACCOUNT
