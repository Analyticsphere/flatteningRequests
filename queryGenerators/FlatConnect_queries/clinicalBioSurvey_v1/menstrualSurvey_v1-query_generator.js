...
const dataSource = {
  dev: 'nih-nci-dceg-connect-dev.Connect.clinicalBioSurvey_v1',
  stg: 'nih-nci-dceg-connect-stg-5519.Connect.clinicalBioSurvey_v1',
  prod: 'nih-nci-dceg-connect-prod-6d04.Connect.clinicalBioSurvey_v1',
};
const pathToConceptIdList = require('./clinicalBioSurvey_v1-lists.json');
let variables = fs
  .readFileSync('./clinicalBioSurvey_v1-variables.csv', 'utf8')
  .split(/\r?\n/)
  .map((v) => v.trim())
  .filter((v) => v.length > 0);
...
""";
  ...
  CREATE OR REPLACE TABLE FlatConnect.clinicalBioSurvey_v1_JP AS (
  WITH
  json_data AS (
    SELECT
      Connect_ID,
...
ORDER BY Connect_ID
  ...
  fs.writeFileSync(`./FlatConnect.clinicalBioSurvey_v1_JP-${tier}.sql`, content);
}
