...
const pathToConceptIdList = require('./BUM-lists-prod.json');
let variables = fs
  .readFileSync('./BUM-variables-prod.csv', 'utf8')
  .split(/\r?\n/)
  .map((v) => v.trim())
  .filter((v) => v.length > 0);
...
ORDER BY Connect_ID
  ...
  fs.writeFileSync(`./bloodUrineMouthwash_JP_SA-${tier}.sql`, content);
}
