...
const pathToConceptIdList = require('./recruitment-lists');
let variables = fs
  .readFileSync('./recruitment-variables.csv', 'utf8')
  .split(/\r?\n/)
  .map((v) => v.trim())
  .filter((v) => v.length > 0);
...
""";
  ...
  CREATE OR REPLACE TABLE recruitment.recruitment1_WL AS (
  WITH
  json_data AS (
    SELECT
      Connect_ID,
...
ORDER BY Connect_ID
  ...
  fs.writeFileSync(`./recruitment1_WL-${tier}.txt`, content);
}
...
function generateSelects(variables = [], rowName = 'row') {
  ...
  for (let variable of variables) {
    const columnName = variable.replaceAll('.', '_').replace('state_', ''); // remove 'state_'
    selectList.push(
