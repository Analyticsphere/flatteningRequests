...
const pathToConceptIdList = require('./M1V2-lists.json');
let variables = fs
  .readFileSync('./M1V2-variables.csv', 'utf8')
  .split(/\r?\n/)
  .map((v) => v.trim())
  .filter((v) => v.length > 0);
...
""";
  ...
  CREATE OR REPLACE TABLE M1.flatM1V2_JP_SA AS (
  WITH
  json_data AS (
    SELECT
      Connect_ID,
...
ORDER BY Connect_ID
  ...
  fs.writeFileSync(`./flatM1V2_JP_SA-${tier}.sql`, content);
}
