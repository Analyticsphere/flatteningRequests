...
tables   <- list(
  ...
  prom = "Connect.promis_v1")

########### DEFINE PARAMETERS ##################################################
tiers_to_update    <- 'dev' # GCP tiers to update sch. queries for
...
resource_name <- query_config$resource_name[tiers_to_update]

# Extract the transfer configuration ID
transfer_config_id <- 
  tail(strsplit(resource_name[[tiers_to_update]], "/")[[1]], 1)
