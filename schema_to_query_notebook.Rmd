...
# BQ Source Tables
tables   <- list(b    = "Connect.biospecimen",          bs   = "Connect.bioSurvey_v1", 
                 cbs  = "Connect.clinicalBioSurvey_v1", ms   = "Connect.menstrualSurvey_v1",                   m1v1 = "Connect.module1_v1",           m1v2 = "Connect.module1_v2",  
...
projects_to_update <- c(projects$prod) # projects to update sch. queries for
...
filter_vars_from_schema(project,table,schema_csv,out_csv,out_json,output_file_type="json")
```
...
not be flattened as "d_cid_d\_178420302" and have a value of 1. It will instead
...
from Quest with unexpected data structure.
...
```{r}
#TODO Modify such that query can be generated for each tier
#TODO Add option to take query_config as variable
query <- generate_flattening_query(destination_table = query_config$output_table,
                                   table_description = query_config$table_description,
                                   source_table      = query_config$data_source,
...
Sys.setenv(PY_FUNC = py_func, RUN_NOW = run_now)

# system("gcloud auth login") # Use this to authenticate if needed
for (project_id in projects_to_update){
  
  # Make sure resource_name in query_config is correct/unique for a given tier
